...
```
> **`--mode`**(兩種):`gated` / `auto`。
> **`--stage`**(執行哪一段):`all`(預設)/ `plan`(只生成)/ `execute`(只執行,gated review 完用這個)/ `reset-plan`(清空規劃、重跑 plan\_loop)/ `reset-execute-state`(保留規劃,只重置執行狀態)。
...
- **規劃書不滿意，想重新規劃？** 用 `--stage reset-plan`：清空 `state.json` 的 `plan`/`phases` 欄位、刪除 `phases/*.md`，保留執行期狀態與 issues，然後立即重跑 `plan_loop`（以 gated 模式）。改完 `REQUIREMENTS.md` 後再跑這個，就能從頭規劃而不影響已記錄的問題。
  ```bash
  python3 $FW/engine/run.py --stage reset-plan
  ```
- **執行跑過了，想讓 agent 從某段重來？** 用 `--stage reset-execute-state`：保留已收斂的規劃書與 `phases/*.md`，只把執行進度歸零。預設會從第一個 phase 開始全部 reset；也可以指定從哪個 phase 或 task 往後重跑。
  ```bash
  python3 $FW/engine/run.py --stage reset-execute-state
  python3 $FW/engine/run.py --stage reset-execute-state --reset-to-phase 2
  python3 $FW/engine/run.py --stage reset-execute-state --reset-to-phase 2 --reset-to-task TASK-03
  ```
  reset 會把目標範圍內的 task 狀態改回 `TODO`、`conv` 歸零、清掉 phase 驗證計數與卡住/交人旗標，並把 `current_phase` 移回指定 phase；目標 phase 之前的進度會保留。
