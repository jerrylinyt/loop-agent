...
- **每輪固定只讀**:`state.json`(全文)+ `當輪那一節 phase 規格` + `當輪用到的那一份 rules 協定` + `任務宣告的少數依賴檔(且只讀宣告的行範圍)`。
...
- `loop.log` 是**寫給人 `tail -f` 的**,**agent 一律禁止讀取**;引擎也**不 parse log**,只讀 `state.json` 與必要結構欄位(不載入 LLM context)。
...
| 增長源 | 問題 | 對策 |
|--------|------|------|
| **內嵌執行日誌** | 每輪 append → 狀態檔膨脹 → STEP 0 全讀會越讀越爆 | **移出狀態檔**:日誌 append 到 `loop.log`(agent 不回讀)；狀態只留必要摘要欄位 |
| **狀態表 / coverage 表** | 任務多時狀態上下文膨脹 | 大專案把詳細規格放 phase 檔；`state.json` 只留當前判斷需要的結構化欄位與跨階段彙總 |
| **Issue / 修正記錄** | 累積無上限 | **一 Issue 一檔**(issues/issue-*.md)；`state.json` 只留未關閉 Issue 的索引與 blocking 計數,不貼內文 |
...
- **`state.json` 保持「決策最小集」**:只放「決定本輪做什麼 + 活計數器 + 索引」;規格放 phases、方法論放 rules、明細放產出檔、歷史放 log。
- **設定不重複**:同一門檻只在 config 出現一次,避免兩處 drift 也避免狀態檔變胖。
...
日誌移出 `state.json`、Issue/產出分散成檔、log rotation 已設、每輪讀取預算明確、沒有「整批讀資料來檢視」的任務。
