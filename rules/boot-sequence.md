...
> 相關規則:`git-safety.md`(STEP G)、`state-model.md`(計數器/流程)、`context-budget.md`(讀取預算)、`convergence.md`、`completeness.md`、`oscillation-escalation.md`、`issues.md`。
...
STEP G ▶ 【Git 完整性檢查 — 最先做,防檔案被弄壞】
   依 git-safety.md 執行 G0~G4:
     G0. 親自讀 `git diff HEAD~1 HEAD`,用「語意」審查上一次 commit 是否合理
         (有無內容被無故砍掉/截斷;改動與該輪任務是否相符)。不合理 → git revert --no-edit HEAD + 開 Issue。
     G1. 若工作區 .git 不存在 → 記住稍後要 git init(初始化任務)。
     G2. 主控檔「整檔空白」粗篩 → 損壞就 git checkout HEAD -- <該檔>。
     G3. 懷疑其他產出檔被弄壞 → git status 逐一處理(❌ 不用 reset --hard / clean -fd / checkout .)。
     G4. 確認工作區乾淨可信才往下。
   ⚠️ git 只作用於「工作區(code repo)」;framework_path 是外部唯讀框架,絕不寫入、不納入 git。
