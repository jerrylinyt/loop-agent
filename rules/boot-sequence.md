...
STEP 9 ▶ 更新狀態。狀態唯一的單一事實來源為 state.json，**嚴禁直接修改 state.json 本體**。
         ...
         ⚠️ 務必回填三個「震盪偵測」欄位（引擎決策關鍵）：
           · `{state_cli} set last_round_mode <推進|驗證>`
           · `{state_cli} set last_round_result <PASS|FAIL|NA>`
           · `{state_cli} set last_round_fail_tasks "<以逗號分隔的任務ID>"`（PASS 留空）
         ...
         若本輪因震盪被升級到人類(stuck_level=2):依 oscillation-escalation.md 開 BLOCKING Issue、
            ...
            若已無其他可做任務則設 `{state_cli} set human_required true`。
         每次呼叫 CLI 修改後，狀態會被安全原子寫入 state.json，人類可以使用 Web Dashboard 查看最新狀態看板。



STEP C ▶ 【Git 提交 — 本輪的還原點】(只在工作區 / code repo)
         commit 前先過 .loop/rules/pre-commit-checklist.md 的 6 項自查（機械確認，快速）。
