...
| `NON_BLOCKING` | 明確外部依賴,或已有人工批准可帶著完成的風險 | ❌ 允許留存 |
...
>    只有「明確外部依賴」或「人類已明確批准可接受的風險」才可列 `NON_BLOCKING`,且須在 Issue 內寫明外部依賴或人工批准紀錄。
>    ❌ 嚴禁:為了讓停止條件成立(`blocking_issues==0`)、或為了避免 `consecutive_pass` 歸零,
>    把一個「不解決就無法正確完成」的問題降級成 NON_BLOCKING;也嚴禁 agent 自行判斷某風險「可接受」。
>    分不清屬於哪級、或沒有人工批准紀錄 → 從嚴算 BLOCKING。
