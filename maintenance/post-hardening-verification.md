...
2. 文件即狀態、可冷接手。
3. 不信單次 → 收斂要有**可稽核證據**（獨立重推、留痕、實質差異歸零），不是自我回報。
...
### 規則層（commit 664a634）
1. **全量驗證留痕**（boot-sequence.md STEP 4）：`p_pass+=1` 前是否強制寫驗證證據檔（逐項檔:行＋build/test 原始輸出）？無證據檔能不能還是 +1？
...
5. **集合穩定重列舉留痕**（completeness.md F）：能不能先看舊 inventory 就宣稱「無新項」converge+1？
