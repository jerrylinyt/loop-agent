...
| 腳本 | 階段 | 做什麼 |
|------|------|--------|
| `plan_loop.py` | ② 生成 | 每個 cycle 兩輪：Round A 生成（獨立重推/精修規劃書）、Round B 審查（**獨立 context，只審不生**，跑 Plan Gate）。直到「連續 N 個 cycle 無實質變更且 Gate PASS」→ 收斂；卡死則升級【增強模型】→ 仍卡交人類。狀態落 `.loop/state.json`，log 落 `.loop/plan.log`。 |
| `loop.py` | ③ 執行 | 反覆觸發 agent 依規劃書執行，直到最終結果收斂或交人類。決策狀態在 `.loop/state.json`，log 落 `.loop/loop.log`。 |
...
`.loop/<name>/.loop_state/run.lock`)：同一個 workspace 不會被手滑啟動裝兩次(避免互踩同一份
state/git 狀態)。鎖檔**每輪心跳更新 mtime**(`touch_run_lock`)，殘留判定門檻為
`max(3600, 3×round_timeout)`——所以正常跑的長迴圈(>1h)不會被誤判成殘留而被第二個程序搶鎖並行;
只有真正異常結束留下的鎖才會被自動接手。這跟並行無關,純粹是冪等保護。

## loop.py（執行引擎）它做什麼
反覆觸發 coding agent 依 `state.json` + `phases/*.md` 執行，並負責：
- **config cascade**：框架預設 < 專案 `.loop/loop.config.yaml`。
- **N 階段流程控制**：停止條件、Phase Gate 全依 `config.phases`（最後一筆=最終階段），不寫死階段數。
- **震盪偵測 + 三層升級**：三種卡法任一達門檻都走預設→增強→人類——
  ①失敗指紋架構歷史(偵測「改A壞B」震盪) ②`rounds_since_progress`(不收斂)
  ③**無活動偵測**(活動簽章=phase+各 pass 總和+HEAD,連續多輪不變;另以 `killed_streak`
  防『中斷留半套被 commit 騙過簽章』——抓 CLI 逾時/空轉,不再無聲燒到 max_rounds)。
  失敗指紋與進度/活動標記都**持久化於 `.loop/.loop_state/rounds.jsonl` 的 typed records / `round_finished` 欄位，重啟接續不歸零**。
...
- 狀態欄位（`state.json` 的 `plan` 物件）：`plan_status` `plan_stable_rounds` `plan_model_tier` `plan_rounds_since_progress` `plan_enhanced_rounds_used` `plan_human_required`。
...
`stage=execute` 時 `.loop/<name>/state.json` 是否存在（`phases` 缺則視 stage 決定 error/warning）。
有 error 直接擋下（return 1），不會空轉到 max_rounds 才發現設定沒填對。

## 相印
- Python 3.8+（標準庫即可）。
- `pyyaml` **可選**：有就用它解析 config；沒有則用內建的輕量 YAML 子集解析器（已涵蓋本框架 config 的結構）。
...
# 控制狀態現在已統一為 .loop/<name>/state.json
python3 <framework_path>/engine/loop.py --workspace <name>
# 詳細輸出已直接印在這個終端機；想看歷史/背景執行才用：
tail -f .loop/<name>/loop.log
```

可用環境變數（僅限於執行期單次控制，不會覆寫專案設定檔）：`LOOP_CONFIG`（config 路徑，優先於 `--workspace`）、`LOOP_WORKSPACE`（同 `--workspace`）、
`LOOP_QUIET=1`（同 `--quiet`）。
...
- `models.default` / `models.enhanced`：兩層模型（卡住時切 enhanced）。env `DEFAULT_MODEL`/`ENHANCED_MODEL` 可覆盖。
- `prompts.base` / `prompts.escalation` / `prompts.plan` / `prompts.plan_gate`：提示樣板（省略用框架預設）。
  佔位：`{control}` `{framework}` `{requirements}`。`plan_gate` 是獨立審查輪用的（唯讀，不可生成/修改規劃書）。
