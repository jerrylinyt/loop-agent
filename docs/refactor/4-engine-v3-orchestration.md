...
```jsonc
{
  ...
  "phases": [{
    ...
    "tasks": [{
      ...
      "verify": {                                       // ★ 驗證契約（見 M3）
        "kind": "command",                              // command | reverify | enumerate | none | manual
        "check": "npm test -- orders.spec.ts",          // kind=command 必填
        "threshold": 2                                  // kind=reverify/enumerate 用（執行期門檻的單一事實來源=本欄位；
      },                                                //   phases[].converge_threshold 僅為 plan 期預設值輸入，引擎不讀）
                                                        // kind=manual：acceptance-standards §6 的主觀品質項——引擎不驗證、
                                                        //   agent 完成後照 none 收斂（不擋機械停止），但引擎標記並強制列入
                                                        //   RUN_REPORT「待你裁決」與 accept gate 的人審清單（dashboard ✋ 圖示對應此值）
...
3. Phase Gate：current_phase 的任務全 CONVERGED ∧ gate.pass≥required ∧ blocking==0 ∧ 非最後 phase
   ...
   （執行後回到步驟 1 重新評估，同一次迴圈內完成，省掉 v2「過 gate 燒一輪」的開銷；
     並寫一筆 rounds.jsonl record `type: phase_advanced`——dashboard 事件標記層 🔁 的資料源）
4. 掃 current_phase 任務（order 排序）取第一個「可做」者：
   ...
   └ DRAFTED:
       ...
       verify.kind == manual    → 同 none 收斂（不擋機械停止），但引擎標記 manual、
                                  強制列入 RUN_REPORT 人審清單與 accept gate（見 M1.1 註）
5. 無可做任務：
...
4. **記錄欄位規格（任務歸因，收官重審補入——本處是實作點，計畫書 5 T1 為消費/驗證方）**：
   - 每筆 `round_finished` 必含 `task` / `action` / `verify_kind` / `model`（PHASE_VALIDATE 與 phase 級 check 輪 task 為 null）。
   - `review_revert` record 必含 `round`（被退輪次）與 `task`（引擎從該輪的任務卡落檔 `cards/R{n}.md` / expected_action 取得，非推斷）——analytics 的「任務 REVERT 次數」據此可算。
   - 標 FROZEN 時寫 `type: task_frozen` record（含 task、觸發訊號摘要）——dashboard 事件標記層 🧊 的資料源。
...
| 引擎 commit 豁免 | 依 docs#3 T7 引擎自產 commit 規約：`loop-engine(<類別>):` 前綴 ∧ diff 完全落在該類路徑白名單 → 該 commit 從審查範圍排除（前綴對但路徑越界 → 照審；agent 格式 commit 永不豁免）——規約在 M4 改寫規則檔時**必須**隨遷至此，不得遺失 |
