# 🧭 ROUTING — Prompt 與產出物路由總表（單一稽核點）

> **這份文件是什麼**：整個 refactor 系列（docs/refactor 1–5 + dashboard/refactor 1–4）引入了大量產出物與注入通道。本表回答兩個問題：
> **(A) 每一種被喚醒的 agent，拿到什麼、從哪個通道拿？** **(B) 每一個產出物，誰生產、誰消費、進不進 agent context？**
> **維護規則**：任何計畫書若新增/變更 prompt 注入或產出物，**必須同步更新本表**（執行 agent 的 PR 檢查項）。表的正確性由 CI「參照完整性測試」機械保證（計畫書 4 M5 第 8 點）——prompt 指向的檔案/錨點不存在即紅。
