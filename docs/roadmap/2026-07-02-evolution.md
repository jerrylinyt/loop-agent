...
**知識體系的三層分工**（前兩層已排入 refactor 計畫，本項是第三層）：
...
三層都刻意**不用向量檢索**：抽取端弱模型萃取是雜訊庫、loop 邊跑邊改 code 而向量庫無時效感知、embedding 檢索的漏抓/誤抓是靜默的——皆與「可 grep、可追溯、可稽核」的體質相沖。

**升級為真檢索系統的判準**（數據驅動，不憑感覺）：analytics（計畫書 5）顯示 agent 反覆重讀/重推導同一批已分析來源（同檔案出現在多個不相干任務的實際讀取 trace、或重驗差異揭露重複推導錯誤），**且** INDEX 已大到超出注入預算（如分析文件上千份、過濾注入也不夠用）——屆時第一步是**分層索引**（索引的索引），embedding 永遠是最後手段。
